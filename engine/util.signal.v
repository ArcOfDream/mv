...
pub fn (mut t SignalTable) emit(signal string, sender voidptr, args []SignalArg, vm ?&wren.VM, call_handles []?&wren.Handle) {
	// Snapshot is used only for dispatch - so handlers connected or
	// disconnected mid-dispatch don't affect the current firing pass.
	...
	// filter the LIVE map entry, not the snapshot.
	// this preserves any connect/disconnect calls made during dispatch.
	if has_once {
		if live := t.handlers[signal] {
			t.handlers[signal] = live.filter(fn (h SignalHandler) bool {
				if h is VSignalHandler {
					return !h.once
				}
				return true
