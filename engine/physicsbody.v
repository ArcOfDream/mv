...
/*
PhysicsBody: usage example

Collision layers and masks are 32-bit bitmasks (1-indexed). A body is detected
by a query only when the querying body's mask overlaps this body's layer.
The default for both is 1, so bodies collide with everything by default.

	// player on layer 1, scans layers 1 and 2 (enemies and world)
	mut player := PhysicsBody.new(app, 'player', .kinematic, physics.AABB{
		min: physics.Vec{-8, -16}
		max: physics.Vec{ 8,  16}
	})
	player.set_layer(1)
	player.set_mask(1)
	player.set_mask(2)

	// enemy on layer 2, only scans layer 1 (player)
	mut enemy := PhysicsBody.new(app, 'enemy', .kinematic, physics.AABB{
		min: physics.Vec{-8, -8}
		max: physics.Vec{ 8,  8}
	})
	enemy.set_layer(2)
	enemy.clear_mask(1)  // clear default
	enemy.set_mask(1)

	// static wall: registers once at ready, never re-registers
	mut wall := PhysicsBody.new(app, 'wall', .static_body, physics.AABB{
		min: physics.Vec{0, 0}
		max: physics.Vec{16, 128}
	})

move_and_collide returns all overlapping hits without moving the body.
Use it when you want raw collision data and will handle resolution yourself.

	hits := player.move_and_collide(velocity * dt)
	for hit in hits {
		println('hit ${hit.other.name()}: normal ${hit.normal}, depth ${hit.depth}')
	}

move_and_slide moves the body, projecting velocity along hit surfaces.
Collisions are stored in slide_collisions for the current frame.
Returns the remaining (unspent) velocity after all slides.

	remaining := player.move_and_slide(velocity * dt, 4)
	player.set_global_pos(player.get_global_pos() + remaining)
	...
	if n := player.get_floor_normal() {
		// slope-aware movement: align velocity to surface
	}

For one-frame collision queries without movement (e.g. trigger zones):

	test_shape := physics.AABB{
		min: physics.Vec{px - 4, py - 4}
		max: physics.Vec{px + 4, py + 4}
	}
	candidates := app.physics_world.hash.query_shape(test_shape)
	for id in candidates {
		other := app.bodies[id] or { continue }
...
pub fn PhysicsBody.instantiate(app &App, name string) &PhysicsBody {
	return &PhysicsBody{
		app:       app
		node_name: name
	}
