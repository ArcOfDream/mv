...
**`Vec2`**: the engine's primary 2D vector type, aliased directly to Raylib's `C.Vector2` to avoid conversion overhead at rendering boundaries. Carries the usual arithmetic operators (`==` uses exact bitwise comparison; use `approx_eq(epsilon)` for tolerant checks), `dot`, `cross`, `normalize`, `length`, `lerp`, and angle helpers.
...
**`GameState`**: a lightweight typed key-value store for global runtime values. Values are stored as the sum type `StateValue = bool | f32 | i64 | string`, retrieved with `get[T](key)` which returns `?T`. Carries the current frame delta via `dt() f32` / `set_dt(f32)` (set once per frame by the engine) and a `user_data voidptr` slot for attaching an arbitrary typed context via `set_user_data[T]` / `get_user_data[T]`. Useful for passing frame-level state to systems that don't hold a direct `App` reference.

**`StringName`**: a lightweight handle to an interned string stored in a `StringNameMap`. Two `StringName` values pointing to the same interned entry compare equal via pointer identity (`==`) rather than character-by-character, making them safe and cheap to use as map keys in hot paths. Entries persist for the map's lifetime with no per-handle refcounting. Used by `InputMap` for action names and available for any system that keys on string labels at runtime.
