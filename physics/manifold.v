	...
	match s1 {
		...
		AABB {
			match s2 {
				...
				AABB    { aabb_to_aabb_manifold(s1, s2, mut m) }
				Capsule { aabb_to_capsule_manifold(s1, s2, mut m) }
				Polygon { aabb_to_poly_manifold(s1, &s2, &xtransform_identity, mut m) }
				else    {}
			}
		}
		Capsule {
			match s2 {
				...
				Capsule { capsule_to_capsule_manifold(s1, s2, mut m) }
				Polygon { capsule_to_poly_manifold(s1, &s2, &xtransform_identity, mut m) }
				else    {}
			}
		}
		Polygon {
			match s2 {
				...
				Polygon { poly_to_poly_manifold(&s1, &xtransform_identity, &s2, &xtransform_identity, mut m) }
				else    {}
			}
		}
		else {}
