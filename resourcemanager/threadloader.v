...
pub fn (tl &ThreadLoader) is_closed() bool { return tl.closed }
...
fn (tl &ThreadLoader) dispatch_rres(cmd LoadCommand, src RresSource) {
    ...
    match cmd.kind {
        .texture {
            if chunk := loader.load_single(src.key) {
                ...
                if rl.is_image_valid(img) {
                    tl.events <- LoadEvent{ name: cmd.name, content: img }
                    return
                }
            }
        ...
        .sound {
            if chunk := loader.load_single(src.key) {
                ...
                if rl.is_wave_valid(wave) {
                    tl.events <- LoadEvent{ name: cmd.name, content: wave }
                    return
                }
            }
        ...
        .shader {
            ...
            tl.events <- LoadEvent{ name: cmd.name, content: ShaderFile{ vs: vs, fs: fs } }
        }
        .data {
            if chunk := loader.load_single(src.key) {
                defer { chunk.unload() }
                bytes, size := rres.load_data_from_resource(chunk)
                tl.events <- LoadEvent{ name: cmd.name, content: unsafe { arr.carray_to_varray[u8](bytes, int(size)) } }
                return
            }
...
fn (tl &ThreadLoader) dispatch_file(cmd LoadCommand, src FileSource) {
    match cmd.kind {
        ...
        .data {
            ...
            tl.events <- LoadEvent{ name: cmd.name, content: bytes }
        }
    }
}

fn (tl &ThreadLoader) send_err(name string, msg string) {
    tl.events <- LoadEvent{ name: name, err: msg }
}
