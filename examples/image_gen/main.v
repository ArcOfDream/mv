...
fn (mut g Game) init() {
	if mut app := g.app {
		...
		mut rainbow := mv.Gradient.from_colors([
			rl.red, rl.orange, rl.yellow,
			rl.green, rl.skyblue, rl.darkblue, rl.violet,
		])
		...
		app.textures.load_from_image('fill',
			mv.gen_image_fill(img_size, img_size, rl.Color{30, 160, 180, 255}))
		...
		app.textures.load_from_image('grid',
			mv.gen_image_grid(img_size, img_size, 4, 4, 2,
				rl.Color{30, 30, 30, 255}, rl.Color{100, 200, 120, 255}))

		// [3] linear horizontal: sunset
		app.textures.load_from_image('linear_h',
			mv.gen_image_gradient_linear(img_size, img_size, &sunset,
				Vec2{0.0, 0.5}, Vec2{1.0, 0.5}))

		// [4] linear vertical: sky
		app.textures.load_from_image('linear_v',
			mv.gen_image_gradient_linear(img_size, img_size, &sky,
				Vec2{0.5, 0.0}, Vec2{0.5, 1.0}))

		// [5] linear diagonal
		app.textures.load_from_image('linear_diag',
			mv.gen_image_gradient_linear(img_size, img_size, &rainbow,
				Vec2{0.0, 0.0}, Vec2{1.0, 1.0}))

		// [6] radial vignette
		app.textures.load_from_image('radial',
			mv.gen_image_gradient_radial(img_size, img_size, &vignette,
				Vec2{0.5, 0.5}, 0.5))

		// [7] radial focal: sphere shading
		app.textures.load_from_image('radial_focal',
			mv.gen_image_gradient_radial_focal(img_size, img_size, &sphere_grad,
				Vec2{0.5, 0.5}, 0.5, Vec2{0.35, 0.3}))

		// [8] conic: rainbow sweep
		app.textures.load_from_image('conic',
			mv.gen_image_gradient_conic(img_size, img_size, &rainbow,
				Vec2{0.5, 0.5}, 0.0))

		// [9] xor: rainbow
		app.textures.load_from_image('xor_rainbow',
			mv.gen_image_xor(img_size, img_size, &rainbow))

		// [10] xor: two-tone
		app.textures.load_from_image('xor_two',
			mv.gen_image_xor(img_size, img_size, &two_tone))

		// [11] sdf circle: soft glow
		app.textures.load_from_image('sdf_glow',
			mv.gen_image_sdf_circle(img_size, img_size,
				Vec2{0.5, 0.5}, 0.35, 0.15, &glow))

		// lay out sprites in a 4×3 grid

		names := [
			'fill',        'checker',     'grid',         'linear_h',
			'linear_v',    'linear_diag', 'radial',       'radial_focal',
			'conic',       'xor_rainbow', 'xor_two',      'sdf_glow',
		]
